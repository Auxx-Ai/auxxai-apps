// src/blocks/slack/shared/use-slack-data.ts

/**
 * Module-level cache + React hook for loading Slack data (channels, users).
 *
 * Uses a module-level Map cache that persists as long as the SDK iframe
 * is alive (survives panel open/close). Entries expire after 5 minutes.
 *
 * Supports an `enabled` option for lazy loading — data is only fetched
 * when the active resource/operation needs it.
 */
