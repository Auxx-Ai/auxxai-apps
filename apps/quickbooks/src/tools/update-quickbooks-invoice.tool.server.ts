// src/tools/update-quickbooks-invoice.tool.server.ts
...
interface UpdateQuickbooksInvoiceInput {
  invoiceId: string
  lines?: {
    itemId: string
    amount: number
...
interface UpdateQuickbooksInvoiceOutput {
  invoiceId: string
...
export default async function updateQuickbooksInvoice(
  input: UpdateQuickbooksInvoiceInput
): Promise<UpdateQuickbooksInvoiceOutput> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      const result = await quickbooksApi<any>(realmId, '/invoice?operation=update', credential, {
        method: 'POST',
        body,
        sandbox,
      })
      ...
      if (/5010|stale|StaleObject/i.test(msg)) {
        const e = new Error(
          'QuickBooks invoice was modified concurrently. Re-fetch and retry.'
