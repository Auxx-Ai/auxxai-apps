// src/blocks/supabase/shared/use-supabase-data.ts

/**
 * Module-level cache + React hook for loading Supabase data (tables, columns).
 *
 * Uses composite cache keys that include parent dependencies (e.g.,
 * `columns:${schema}:${table}`) so switching a parent selector naturally
 * loads fresh child data.
 *
 * Cache entries expire after 5 minutes. The cache persists as long as the
 * SDK iframe is alive (survives panel open/close).
 */
