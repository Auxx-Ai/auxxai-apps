...
//
// Dispatcher for the `google-sheets` workflow block. Routes the user's
...
// input is forwarded through unchanged — the internal `gog_sheets_block_*`
...
export default async function googleSheetsExecute(
  ...
  const toolMap = googleSheetsToolMap as Record<string, string>
  ...
  if (!ctx?.runTool) {
    throw new Error('Google Sheets block requires lambda runtime ctx.runTool')
  }
