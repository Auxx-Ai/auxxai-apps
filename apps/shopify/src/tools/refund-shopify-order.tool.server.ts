...
export default async function refundShopifyOrder(
  ...
  return withOrderId(shopDomain, token, input.shopifyOrderId, async (numericId) => {
    ...
    const txResult = await shopifyApi<{ transactions: any[] }>(
      shopDomain,
      token,
      `/orders/${numericId}/transactions.json`
    )
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
