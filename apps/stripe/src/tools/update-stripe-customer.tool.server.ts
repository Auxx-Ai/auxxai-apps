// src/tools/update-stripe-customer.tool.server.ts
...
interface UpdateStripeCustomerOutput {
  customer: {
    stripeCustomerId: string
...
export default async function updateStripeCustomer(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const raw = await stripeApi<any>('POST', `/customers/${input.stripeCustomerId}`, apiKey, { body })
