// src/blocks/airtable/shared/use-airtable-data.ts

/**
 * Module-level cache + React hook for loading Airtable data (bases, tables, fields, views).
 *
 * Uses composite cache keys that include parent dependencies (e.g., `tables:${baseId}`)
 * so switching a parent selector naturally loads fresh child data.
 *
 * Cache entries expire after 5 minutes. The cache persists as long as the SDK iframe
 * is alive (survives panel open/close).
 */
