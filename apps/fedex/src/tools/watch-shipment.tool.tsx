...
export const watchShipmentTool = defineTool({
  id: 'watch_shipment',
  name: 'Watch FedEx shipment',
  description:
    'Start watching a FedEx shipment for status changes. Validates the number and returns its current status in the same turn, then registers it so the shipment-status-changed trigger fires on future updates (delivered, exception, …). Re-watching refreshes the 30-day expiry.',
  icon,
  inputs: z.object({
